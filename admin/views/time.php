...
[$from, $to, $period_label] = Edifice_Time::get_period_range('month');
$stats      = Edifice_Time::get_period_stats($from, $to);
$by_client  = Edifice_Time::get_by_client($from, $to);
$by_project = Edifice_Time::get_by_project($from, $to);
$entries    = Edifice_Time::get_all(['from' => $from, 'to' => $to]);
$contacts   = Edifice_CRM::get_all();
$projects   = Edifice_Projects::get_all();
$export_url = admin_url('admin-ajax.php') . '?action=edifice_time_export&nonce=' . wp_create_nonce('edifice_nonce') . '&from=' . $from . '&to=' . $to;
?>
<div class="lh-wrap">
  <div class="lh-header">
    ...
    <div id="timer-idle">
      ...
      <div class="lh-timer-start-row">
        ...
        <select id="timer-project" class="lh-input">
          ...
          <?php foreach ($projects as $p): ?><option value="<?= $p['id'] ?>"><?= esc_html($p['name']) ?><?= empty($p['contact_id']) ? ' (intern)' : '' ?></option><?php endforeach; ?>
        </select>
        ...
        <tbody id="time-entries-body">
          ...
          <tr>
            ...
            <td class="actions">
              ...
              <button class="lh-btn lh-btn-danger lh-btn-sm lh-delete-btn" data-action="edifice_time_delete" data-id="<?= $e['id'] ?>">Slett</button>
            </td>
          </tr>
          <?php endforeach; ?>
  ...
  <div class="lh-modal">
    ...
    <div class="lh-modal-body">
      <form class="lh-ajax-form">
        <input type="hidden" name="ajax_action" value="edifice_time_save">
...
(function(){
  ...
  lhAjax('edifice_time_active',{},function(res){if(res&&res.success&&res.data){var t=res.data;showRunning([t.description,t.contact_name].filter(Boolean).join(' · '),t.started_at);}});

  document.getElementById('btn-start-timer').addEventListener('click',function(){
    var sel=document.getElementById('timer-contact');
    lhAjax('edifice_time_start',{description:document.getElementById('timer-description').value.trim(),contact_id:sel.value,project_id:document.getElementById('timer-project').value},function(res){
      if(res&&res.success){var lbl=[document.getElementById('timer-description').value,sel.options[sel.selectedIndex]&&sel.options[sel.selectedIndex].text!='— Klient —'?sel.options[sel.selectedIndex].text:''].filter(Boolean).join(' · ');showRunning(lbl,res.data.started_at);toast('Timer startet!','success');}
      else{toast((res.data&&res.data.message)||'Feil','error');}
    });
  });

  document.getElementById('btn-stop-timer').addEventListener('click',function(){
    lhAjax('edifice_time_stop',{},function(res){
      if(res&&res.success){showIdle();toast('Stoppet — '+res.data.hours+' timer logget.','success');loadPeriod(currentPeriod);}
  ...
  function loadPeriod(p){
    lhAjax('edifice_time_period_data',{period:p},function(res){
